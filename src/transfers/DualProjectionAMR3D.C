...
template<>
InputParameters validParams<DualProjectionAMR3D>()
{
...
DualProjectionAMR3D::DualProjectionAMR3D(const InputParameters & parameters) :
...
{
    _console<<"Constructor of DualProjectionAMR3D "<<name() << std::endl;
...
void
DualProjectionAMR3D::initialSetup()
{
    _console<<"initialSetup of DualProjectionAMR3D "<<name() << std::endl;
...
void DualProjectionAMR3D::execute()
{
...
bool DualProjectionAMR3D::is_inside()
{
    Real xc=_centroid(0);
    Real yc=_centroid(1);
    Real zc=_centroid(2);
...
void DualProjectionAMR3D::constructInterpolationMatrix()
{
    ...
    for (std::map<int,int>::iterator my_it=coarse2fine.begin(); my_it!=coarse2fine.end(); ++my_it)
    {
        ...
        for (std::map<int,Point>::iterator fine_it=fine_point_map.begin(); fine_it!=fine_point_map.end(); ++fine_it)
        {
            ...
            if (norm < 1e-12)
            {
                ...
                {
                    proj_matrix.set(fine_it->first,local_it->first,1.0/8.0);
                }
                //break;
            }
            ...
            for (int i=0; i<6; ++i)
            {
                ...
                if (norm < 1e-12)
                {
                    for (int l=0; l<side[0].n_nodes(); ++l)
                    {
                        ...
                        proj_matrix.set(fine_it->first,l_nodo.id(),1.0/4.0);
                        if (_has_hanging_nodes)
                        {
                            remove_hanging_nodes[0].set(fine_it->first,l_nodo.id(),-1.0/4.0);
                        }
                    }
                }
            }
            ...
            for (int i=0; i<6; ++i)
            {
                ...
                {
                    std::unique_ptr<Elem const> _edge=side[0].side_ptr(ii);
                    Elem const * edge = _edge.get();
                    RealVectorValue B=edge[0].centroid()-fine_it->second;
                    Real normB=std::sqrt(B*B);
                    if (normB < 1e-12)
                    {
                        for (int ll=0; ll<edge[0].n_nodes(); ++ll)
                        {
                            Node const & ll_nodo=edge[0].node_ref(ll);
                            proj_matrix.set(fine_it->first,ll_nodo.id(),1.0/2.0);
                            if (_has_hanging_nodes)
                            {
                                remove_hanging_nodes[0].set(fine_it->first,ll_nodo.id(),-1.0/2.0);
                            }
                        }
                    }
...
void DualProjectionAMR3D::constructReorganizationMatrix()
{
    MeshBase const & coarse_mesh=_coarse_mesh[0];
...
DualProjectionAMR3D::~DualProjectionAMR3D()
{
    if (_dual && _has_hanging_nodes)
    {
